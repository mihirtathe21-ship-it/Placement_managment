...
export const applyToJob = async (req, res, next) => {
  try {
    ...
    if (job.eligibility.minCGPA && req.user.cgpa < job.eligibility.minCGPA) {
      return res.status(400).json({ message: `Minimum CGPA required: ${job.eligibility.minCGPA}` })
    }
    ...
    if (err.code === 11000) return res.status(400).json({ message: 'You have already applied to this job.' })
    next(err)
  }
}
...
export const getMyApplications = async (req, res, next) => {
  try {
    ...
    const applications = await Application.find(filter)
      .populate('job', 'title company type package location status logo driveDate')
      .sort({ updatedAt: -1 })
...
export const updateApplicationStatus = async (req, res, next) => {
  try {
    ...
    await Notification.create({
      ...
      type: status === 'selected' ? 'selected' : status === 'rejected' ? 'rejected' : status === 'shortlisted' ? 'shortlisted' : 'application_update',
...
export const getAllApplications = async (req, res, next) => {
  try {
    ...
    const applications = await Application.find(filter)
      .populate('student', 'name email branch cgpa rollNumber')
