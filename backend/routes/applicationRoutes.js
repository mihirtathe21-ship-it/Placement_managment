...
// ─────────────────────────────────────────────────────────────
// Cloudinary Storage for Resume Upload
// ─────────────────────────────────────────────────────────────
...
router.patch(
  '/:id/status',
  protect,
  authorize('admin', 'tpo', 'recruiter'),
  updateApplicationStatus
)

// ─────────────────────────────────────────────────────────────
// Bulk Shortlist via Excel
// ─────────────────────────────────────────────────────────────
router.post(
  '/bulk-shortlist',
  protect,
    ...
    try {
      ...
      if (!jobId || !Array.isArray(emails) || emails.length === 0) {
        return res.status(400).json({
          message: 'jobId and emails array are required',
        })
      }
        ...
        if (existing) {
          if (existing.status === 'applied') {
            ...
            existing.statusHistory.push({
              ...
              note: 'Bulk shortlisted via Excel',
            })
