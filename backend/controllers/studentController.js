  ...
  try {
    ...
    for (const row of students) {
      try {
        ...
        const hasBacklog  = backlogs > 0   // ← NEW: derive hasBacklog from count
        ...
        if (existing) {
          ...
          updateData.backlogs   = backlogs           // always update count
          updateData.hasBacklog = backlogs > 0       // ← NEW: keep in sync
          ...
          await User.create({
            ...
            passingYear, cgpa, backlogs, hasBacklog, domain,   // ← NEW: hasBacklog added
...
export const getStudents = async (req, res, next) => {
  try {
    const {
      ...
      maxBacklogs, hasBacklog,                          // ← NEW: hasBacklog filter
    ...
    // Filter: max backlog count (e.g. show only students with ≤ 2 backlogs)
    if (maxBacklogs !== undefined && maxBacklogs !== '') {
      filter.backlogs = { $lte: parseInt(maxBacklogs) }
    }
 
    // ← NEW: Filter: show only students WITH backlogs or WITHOUT backlogs
    // hasBacklog=true  → only students who have backlogs
    // hasBacklog=false → only students with no backlogs (clean record)
