...
import { sendJobNotificationEmail } from '../utils/emailService.js'   // ← NEW
    ...
    for (const row of students) {
      try {
        ...
        if (existing) {
          ...
          updateData.hasBacklog = backlogs > 0
          ...
          await User.create({
            ...
            passingYear, cgpa, backlogs, hasBacklog, domain,
            role: 'student', password: hashedPassword,
            isActive: true,
            isVerified: true,   // ← imported students are pre-verified (TPO added them)
          })
          created++
        }
      } catch (err) {
...
export const getStudents = async (req, res, next) => {
  try {
    const {
      search, minCGPA, maxCGPA, branch, passingYear,
      maxBacklogs, hasBacklog,
      domain, page = 1, limit = 20
    } = req.query
    ...
    if (maxBacklogs !== undefined && maxBacklogs !== '') {
      filter.backlogs = { $lte: parseInt(maxBacklogs) }
    }

    if (hasBacklog !== undefined && hasBacklog !== '') {
      filter.hasBacklog = hasBacklog === 'true'
    }
...
// ─── NEW: Notify all verified students about a new job posting ────────────────
// Call this from your Job controller right after a job is created/published
export const notifyStudentsAboutJob = async (job) => {
  try {
    // Get all active, verified students
    ...
    console.log(`📧 Sending job notifications to ${students.length} students for: ${job.company}`)

    // Send emails concurrently with a small batch to avoid Gmail rate limits
    const BATCH_SIZE = 10
    for (let i = 0; i < students.length; i += BATCH_SIZE) {
      const batch = students.slice(i, i + BATCH_SIZE)
      await Promise.allSettled(
        batch.map(student =>
          sendJobNotificationEmail(student.email, student.name, job).catch(err =>
            console.error(`Failed to email ${student.email}:`, err.message)
          )
        )
      )
      // Brief pause between batches to respect Gmail sending limits
    ...
    // Non-fatal — log but don't crash the job creation request
    console.error('notifyStudentsAboutJob error:', err.message)
  }
}